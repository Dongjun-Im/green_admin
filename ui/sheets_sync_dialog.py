...
class SheetsSyncDialog(wx.Dialog):
    ...
    def _build_ui(self) -> None:
        ...
        self.id_input = wx.TextCtrl(panel, value=self.config.spreadsheet_id, name="시트ID")
        ...
        sizer.Add(
            wx.StaticText(
                panel,
                label=(
                    "처음 동기화 시: 브라우저가 열려 본인 구글 계정 로그인 필요.\n"
                    "OAuth 토큰은 data/google_token.json 에 캐시되어 다음부턴 자동.\n"
                    "credentials.json 이 data/google_credentials.json 에 있어야 합니다."
                ),
            ),
            0, wx.ALL, 10,
        )
        ...
        # 동기화 정책 안내
        policy_text = (
            "동기화 정책 (3개 탭을 시트에 자동 추가, 기존 탭은 건드리지 않음):\n"
            "  · 초록앱_alias_매핑 — 양방향 (가장 최근 수정이 이김)\n"
            "  · 초록앱_구독_매트릭스 — 앱 → 시트 (매번 덮어씀)\n"
            "  · 초록앱_거래내역 — 앱 → 시트 (매번 덮어씀)"
        )
        sizer.Add(
        ...
        self.open_btn = wx.Button(panel, label="시트 열기(&O)")
        ...
        for b in (self.sync_btn, self.open_btn, self.close_btn):
    ...
    def _announce_open(self) -> None:
        if self.config.spreadsheet_id:
            speak("구글시트 동기화 화면. 저장된 시트 ID 가 있습니다. 동기화 시작 버튼을 누르세요.")
        else:
            speak("구글시트 동기화 화면. 먼저 구글시트 ID 또는 URL 을 입력해 주세요.")
