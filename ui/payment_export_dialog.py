"""자료실 구독비 — 내보내기 형식 선택 다이얼로그.

XLSX / HTML / TXT 세 형식 중 원하는 것만 골라 한 번에 저장. 기본값은 셋 다
체크. 스크린리더 사용자를 위해 체크박스 라벨에 단축키 + 안내 라인을 위에
StaticText 로 따로 둔다.

호출 패턴:
    with PaymentExportDialog(parent) as dlg:
        if dlg.ShowModal() == wx.ID_OK:
            formats = dlg.selected_formats   # ["xlsx", "html", "txt"] 부분집합
"""
...
class PaymentExportDialog(wx.Dialog):
    ...
    def _build_ui(self) -> None:
        ...
        self.cb_xlsx = wx.CheckBox(
            panel, label="엑셀 (.xlsx) — 3시트 매트릭스(&X)",
        )
        self.cb_html = wx.CheckBox(
            panel, label="HTML 리포트 (.html) — 운영진 공유용(&H)",
        )
