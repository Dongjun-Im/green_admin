...
def run_authentication(program_name: str = "프로그램") -> requests.Session | None:
    """
    초록등대 동호회 인증을 실행한다.
    ...
    Returns:
        성공 시 로그인된 requests.Session, 실패/취소 시 None.
    """
...
def run_authentication_with_user(
    program_name: str = "프로그램",
) -> tuple[requests.Session, str] | None:
    """run_authentication() + 인증된 user_id 도 함께 반환.

    Args:
        program_name: 인증 성공 시 음성으로 안내할 프로그램명.

    Returns:
        (session, user_id) 튜플, 실패/취소 시 None.
    """
    saved = load_credentials()
    if saved:
        saved_user_id, password = saved
        success, authenticator, status = _do_authenticate(
            saved_user_id, password, program_name, silent=True,
        )
        if success:
            return authenticator.session, saved_user_id
        if status == AuthResult.LOGIN_FAILED:
            delete_credentials()
        else:
            return None
    ...
    try:
        while True:
            ...
            if success:
                if should_save:
                    save_credentials(user_id, password)
                return authenticator.session, user_id
