...
### 수정 — 예약 공지 자동 작업이 죽던 문제 (창 모드 EXE 크래시)
- 작업 스케줄러가 등록한 프로그램을 약 10분마다 백그라운드로 실행할 때
  "'NoneType' object has no attribute 'write'" 오류로 작업이 죽었습니다.
- 원인: 배포 EXE 는 콘솔 창이 없는 모드(console=False)라 `sys.stdout` /
  `sys.stderr` 가 `None` 인데, 헤드리스 작업 실행기가 결과·오류를 그 스트림에
  직접 출력하다 `None` 에 쓰기를 시도해 작업 전체가 중단됐습니다. v1.4.0 의
  예약 공지 자동 발송(post_scheduled) 작업이 이 경로에 처음 닿아 표면화됐습니다.
- 수정: 스트림이 `None` 이어도 안전하게 무시하는 출력 헬퍼로 모든 출력 호출을
  교체. 작업 결과는 로그 파일(`data/logs/scheduler_*.log`)에도 남겨 콘솔 없는
  환경에서도 기록이 남도록 했습니다. None 스트림 상태에서 죽지 않는지 회귀
  테스트 추가.

## v1.4.1 (2026-06-12)

### 수정 — 예약 발송 접근성 (v1.4.0 후속 버그픽스)
- **예약 날짜·시각을 스크린리더가 전혀 못 읽던 문제**: '공지 작성' 화면의 예약 발송
  칸에 쓰던 날짜·시각 선택 컨트롤(wx.adv 피커)이 NVDA 등에서 값이 안 읽혔습니다.
  앱 전반의 입력칸 방식(TextCtrl)에 맞춰 일반 텍스트 입력으로 교체 — 날짜는
  `2026-06-15`, 시각은 24시간제 `14:30` 형식으로 직접 입력합니다. 형식이 틀리면
  올바른 형식을 안내합니다.
- **'화면 선택' 라디오에서 아래 방향키가 첫 항목으로 튀던 문제**: 맨 위 화면 선택
  라디오가 가로로 배치돼 위/아래 화살표 이동이 첫 항목으로 점프했습니다. 세로
  배치로 바로잡아 위/아래 화살표로 '게시판 설정 / 공지 작성 / 게시물 관리' 를
  순서대로 이동합니다. '게시판 설정' 을 불러왔을 때 나오는 설정 라디오도 동일하게
  세로 배치로 통일.

## v1.4.0 (2026-06-12)

### 추가 — 예약 공지 발송
- '공지 작성' 화면에 '예약 발송' 추가. 날짜·시각을 정하고 '예약 발송(&Y)' 을
  누르면, 정해진 시각에 공지가 자동으로 게시판에 올라간다. 대상 게시판은 단일
  게시판 입력과 일괄 선택을 합쳐서 잡는다.
- 약 10분마다 도는 백그라운드 작업(post_scheduled)이 도래한 예약을 발송. 예약이
  없을 때는 사이트 로그인조차 하지 않아 가볍게 동작. 첫 예약 시 작업 스케줄러에
  자동 등록(자동 스케줄러 관리 GUI 에도 노출).
- '작업' 메뉴 → '예약 공지 목록(&Q)...' 신규 — 예약 건을 스크린리더 친화 목록으로
  보고 예약 취소·목록 삭제. 줄 맨 앞 [예약]/[완료]/[실패]/[취소] 상태 마커.
- 정해진 시각에 PC 가 켜져 있어야 발송(잠금 화면 OK). 메일과 달리 공지 발송은
  rtgreen 계정이 아니어도 로그인한 관리자 계정으로 동작.

### 수정 — 게시판 관리 라디오 방향키
- '게시판 관리 / 공지 작성' 창 맨 위 '화면 선택' 라디오가 가로 배치라 위/아래
  화살표로 이동할 때 첫 항목으로 튀던 문제 수정. 세로 배치(RA_SPECIFY_ROWS)로
  바꿔 화살표가 세 항목 사이를 순서대로 이동.

### 코드 구조
- `core/scheduled_notice.py` 신규 — 예약 공지 큐 저장소(ScheduledNotice +
  ScheduledNoticeStore, data/scheduled_notices.json). wx·네트워크 의존 없음.
- `ui/scheduled_notice_dialog.py` 신규 — ScheduledNoticeDialog.
- `core/scheduler_runner.py` — 헤드리스 작업 post_scheduled (도래분 없으면
  로그인 전 조기 종료). `core/scheduler_setup.py` — MINUTE/HOURLY 의 /MO 반복
  간격 인자 지원 + post_scheduled DEFAULT_SCHEDULES 항목.

### 매뉴얼
- 신규 챕터 '예약 공지 (정해진 시각에 자동 게시)' — 예약 방법, 약 10분 주기,
  PC 켜짐 전제, 목록에서 취소하는 법.
